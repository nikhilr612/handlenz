...
char* readFile(const char* filename) {
    ...
    // Copy the data from the vector to a dynamically allocated buffer
...
static std::vector<std::vector<std::array<float, 3>>> get_landmarks_new(const std::shared_ptr<mediapipe::LibMP>& hand_landmark) {
	std::vector<std::vector<std::array<float, 3>>> normalized_landmarks;
	// I use a unique_ptr for convenience, so that DeletePacket is called automatically
	// You could also manage deletion yourself, manually:
	// const void* packet = face_mesh->GetOutputPacket("multi_face_landmarks");
	...
	// Keep getting packets from queue until empty
	while (hand_landmark->GetOutputQueueSize("landmarks") > 0) {
		lm_packet_ptr.reset(hand_landmark->GetOutputPacket("landmarks"));
	}
	...
	// std::cout << "Debug: Num Hands " << num_hands << std::endl;
	for (int i = 0; i < num_hands; i++) {
		...
		// std::cout << "Debug: ProtoMsgByteSize = " << lm_list_proto_size << std::endl;
	...
	// Initialize a mediapipe::NormalizedLandmarkList object from the buffer
		...
		std::cout << "Iterating over landmarks..." << std::endl;

		// Copy the landmark data to our custom data structure
		normalized_landmarks.emplace_back();
		for (const mediapipe::NormalizedLandmark& lm : landmarks.landmark()) {
			std::cout << "x " << lm.x() << " y " << lm.y() << " z " << lm.z() << std::endl;
...
int old_main(int argc, char* argv[]) {
	// adapted from https://github.com/google/mediapipe/blob/master/mediapipe/graphs/hand_landmark/hand_desktop_live.pbtxt
...
int main(int argc, char const *argv[]){
	...
	const char* graph = readFile("mediapipe/graphs/hand_tracking/hand_tracking_desktop_live.pbtxt");

	std::shared_ptr<mediapipe::LibMP> hand_landmark(mediapipe::LibMP::Create(graph, "input_video"));
	hand_landmark -> AddOutputStream("landmarks");
	...
	while (cap.read(frame_bgr)) {
		...
		// Feed RGB frame into MP face mesh graph (image data is COPIED internally by LibMP)
		bool success = hand_landmark->Process(frame_rgb.data, frame_rgb.cols, frame_rgb.rows, mediapipe::ImageFormat::SRGB);
		if (!success) {
			std::cerr << "Process() failed!" << std::endl;
			break;
		}

		hand_landmark->WaitUntilIdle();
		...
		// Get landmark coordinates in custom data structure using helper function (see above)
		std::vector<std::vector<std::array<float, 3>>> normalized_landmarks = get_landmarks_new(hand_landmark);
		...
		for (int hand_num = 0; hand_num < num_detected; hand_num++) {
			for (const std::array<float, 3>& norm_xyz : normalized_landmarks[hand_num]) {
				std::cout << "Debug: Visiting Hand: " << hand_num << " norm_xyz " << norm_xyz[0] << ", " << norm_xyz[1] << ", " << norm_xyz[2] << std::endl;
				int x = static_cast<int>(norm_xyz[0] * frame_bgr.cols);
				int y = static_cast<int>(norm_xyz[1] * frame_bgr.rows);
				cv::circle(frame_bgr, cv::Point(x, y), 1, cv::Scalar(0, 255, 0), -1);
			}
